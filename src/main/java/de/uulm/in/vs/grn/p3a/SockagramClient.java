...
public class SockagramClient {
    ...
    public static void main(String[] args){
        ...
        try {
            ...
            sock.sendImage("2",Files.readAllBytes(file));

            file = Paths.get(fileName.insert(fileName.indexOf("."),2).toString()); //füge eine 2 zum datei namen hinzu
    ...
    public void sendImage(String filter, byte[] fileData) throws IOException{
        byte filterByte = Byte.parseByte(filter); //TODO: Implement the filters Type(ist aber noch nicht auf moodle)
    ...
    public byte[] getImageResponse() throws IOException{ //TODO: maybe rewrite mit records funkt aber ist memory ineffizient
    ...
    private byte[] createRequest(byte filter, byte[] data) throws IOException{
        ...
        int headLen = 5;//constant offset for the length in byte of the header
